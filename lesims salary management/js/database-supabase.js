...
const DB = {
    ...
    advances: {
        getAll: async function() {
            try {
                const client = await DB._ensureInitialized();
                const { data, error } = await client.from('advances').select('*').order('date', { ascending: false });
                if (error) handleSupabaseError(error, "advances.getAll");
                return DB.utils.snakeToCamelArray(data || []);
            } catch (error) {
                handleSupabaseError(error, "advances.getAll");
            }
        },
        getById: async function(id) {
            try {
                const client = await DB._ensureInitialized();
                if (!id) return null;
                const { data, error } = await client.from('advances').select('*').eq('id', id).single();
                if (error) {
                    if (error.code === 'PGRST116') return null;
                    handleSupabaseError(error, `advances.getById(${id})`);
                }
                return DB.utils.snakeToCamel(data);
            } catch (error) {
                handleSupabaseError(error, `advances.getById(${id})`);
        ...
        getByEmployeeId: async function(employeeId) {
            try {
                ...
                const { data, error } = await client.from('advances').select('*').eq('employee_id', employeeId).order('date', { ascending: false });
                if (error) handleSupabaseError(error, `advances.getByEmployeeId(${employeeId})`);
                return DB.utils.snakeToCamelArray(data || []);
            } catch (error) {
                handleSupabaseError(error, `advances.getByEmployeeId(${employeeId})`);
            }
        },
        getUnpaidByEmployeeId: async function(employeeId) {
            try {
                const client = await DB._ensureInitialized();
                if (!employeeId) return [];
                const { data, error } = await client.from('advances').select('*').eq('employee_id', employeeId).eq('is_paid', false).order('date', { ascending: false });
                if (error) handleSupabaseError(error, `advances.getUnpaidByEmployeeId(${employeeId})`);
                return DB.utils.snakeToCamelArray(data || []);
            } catch (error) {
                handleSupabaseError(error, `advances.getUnpaidByEmployeeId(${employeeId})`);
            }
        },
        getTotalUnpaidByEmployeeId: async function(employeeId) {
            try {
                ...
                const { data, error } = await client.from('advances').select('amount').eq('employee_id', employeeId).eq('is_paid', false);
                if (error) {
                    handleSupabaseError(error, `advances.getTotalUnpaidByEmployeeId(${employeeId})`);
                    return 0;
                }
        ...
        save: async function(advanceData) {
            try {
                const client = await DB._ensureInitialized();
                const dataToSave = {
                    ...
                    reason: advanceData.reason || null,
                    is_paid: advanceData.isPaid,
                    paid_date: advanceData.paidDate || null
                };
                let resultData, error;
                if (advanceData.id) {
                    const response = await client.from('advances').update(dataToSave).eq('id', advanceData.id).select().single();
                    resultData = response.data;
                    error = response.error;
                    if (error) handleSupabaseError(error, `advances.update(${advanceData.id})`);
                } else {
                    const response = await client.from('advances').insert(dataToSave).select().single();
                    resultData = response.data;
                    error = response.error;
                    if (error) handleSupabaseError(error, "advances.insert");
                }
                return DB.utils.snakeToCamel(resultData);
            } catch (error) {
                handleSupabaseError(error, "advances.save");
            }
        },
        delete: async function(id) {
            try {
                const client = await DB._ensureInitialized();
                if (!id) return false;
                const { error } = await client.from('advances').delete().eq('id', id);
                if (error) {
                    handleSupabaseError(error, `advances.delete(${id})`);
                    return false;
                }
                return true;
            } catch (error) {
    ...
    sanctions: {
        getAll: async function() {
            try {
                const client = await DB._ensureInitialized();
                const { data, error } = await client.from('sanctions').select('*').order('date', { ascending: false });
                if (error) handleSupabaseError(error, "sanctions.getAll");
                return DB.utils.snakeToCamelArray(data || []);
            } catch (error) {
                handleSupabaseError(error, "sanctions.getAll");
            }
        },
        getById: async function(id) {
            try {
                const client = await DB._ensureInitialized();
                if (!id) return null;
                const { data, error } = await client.from('sanctions').select('*').eq('id', id).single();
                if (error) {
                    if (error.code === 'PGRST116') return null;
                    handleSupabaseError(error, `sanctions.getById(${id})`);
                }
                return DB.utils.snakeToCamel(data);
            } catch (error) {
                handleSupabaseError(error, `sanctions.getById(${id})`);
        ...
        getByEmployeeId: async function(employeeId) {
            try {
                const client = await DB._ensureInitialized();
                if (!employeeId) return [];
                const { data, error } = await client.from('sanctions').select('*').eq('employee_id', employeeId).order('date', { ascending: false });
                if (error) handleSupabaseError(error, `sanctions.getByEmployeeId(${employeeId})`);
                return DB.utils.snakeToCamelArray(data || []);
            } catch (error) {
                handleSupabaseError(error, `sanctions.getByEmployeeId(${employeeId})`);
            }
        },
        getByMonth: async function(year, month) {
            try {
                ...
                const { data, error } = await client.from('sanctions').select('*').gte('date', startDate.toISOString()).lte('date', endDate.toISOString());
                if (error) handleSupabaseError(error, `sanctions.getByMonth(${year}-${month})`);
                return DB.utils.snakeToCamelArray(data || []);
            } catch (error) {
                handleSupabaseError(error, `sanctions.getByMonth(${year}-${month})`);
            }
        },
        save: async function(sanctionData) {
            try {
                const client = await DB._ensureInitialized();
                const dataToSave = {
                    employee_id: sanctionData.employeeId,
                    date: sanctionData.date,
                    type: sanctionData.type,
                    amount: sanctionData.amount,
                    reason: sanctionData.reason || null
                };
                let resultData, error;
                if (sanctionData.id) {
                    const response = await client.from('sanctions').update(dataToSave).eq('id', sanctionData.id).select().single();
                    resultData = response.data;
                    error = response.error;
                    if (error) handleSupabaseError(error, `sanctions.update(${sanctionData.id})`);
                } else {
                    const response = await client.from('sanctions').insert(dataToSave).select().single();
                    resultData = response.data;
                    error = response.error;
                    if (error) handleSupabaseError(error, "sanctions.insert");
                }
                return DB.utils.snakeToCamel(resultData);
            } catch (error) {
                handleSupabaseError(error, "sanctions.save");
            }
        },
        delete: async function(id) {
            try {
                const client = await DB._ensureInitialized();
                if (!id) return false;
                const { error } = await client.from('sanctions').delete().eq('id', id);
                if (error) {
                    handleSupabaseError(error, `sanctions.delete(${id})`);
                    return false;
                }
                return true;
            } catch (error) {
    ...
    debts: {
        getAll: async function() {
            try {
                const client = await DB._ensureInitialized();
                const { data, error } = await client.from('debts').select('*').order('date', { ascending: false });
                if (error) handleSupabaseError(error, "debts.getAll");
                return DB.utils.snakeToCamelArray(data || []);
            } catch (error) {
                handleSupabaseError(error, "debts.getAll");
            }
        },
        getById: async function(id) {
            try {
                const client = await DB._ensureInitialized();
                if (!id) return null;
                const { data, error } = await client.from('debts').select('*').eq('id', id).single();
                if (error) {
                    if (error.code === 'PGRST116') return null;
                    handleSupabaseError(error, `debts.getById(${id})`);
                }
                return DB.utils.snakeToCamel(data);
            } catch (error) {
                handleSupabaseError(error, `debts.getById(${id})`);
        ...
        getByEmployeeId: async function(employeeId) {
            try {
                const client = await DB._ensureInitialized();
                if (!employeeId) return [];
                const { data, error } = await client.from('debts').select('*').eq('employee_id', employeeId).order('date', { ascending: false });
                if (error) handleSupabaseError(error, `debts.getByEmployeeId(${employeeId})`);
                return DB.utils.snakeToCamelArray(data || []);
            } catch (error) {
                handleSupabaseError(error, `debts.getByEmployeeId(${employeeId})`);
            }
        },
        getUnpaidByEmployeeId: async function(employeeId) {
            try {
                const client = await DB._ensureInitialized();
                if (!employeeId) return [];
                const { data, error } = await client.from('debts').select('*').eq('employee_id', employeeId).eq('is_paid', false).order('date', { ascending: false });
                if (error) handleSupabaseError(error, `debts.getUnpaidByEmployeeId(${employeeId})`);
                return DB.utils.snakeToCamelArray(data || []);
            } catch (error) {
                handleSupabaseError(error, `debts.getUnpaidByEmployeeId(${employeeId})`);
            }
        },
        getTotalUnpaidByEmployeeId: async function(employeeId) {
            try {
                const client = await DB._ensureInitialized();
                if (!employeeId) return 0;
                const { data, error } = await client.from('debts').select('amount').eq('employee_id', employeeId).eq('is_paid', false);
                if (error) {
                    handleSupabaseError(error, `debts.getTotalUnpaidByEmployeeId(${employeeId})`);
                    return 0;
                }
                return (data || []).reduce((sum, row) => sum + (row.amount || 0), 0);
            } catch (error) {
        ...
        save: async function(debtData) {
            try {
                const client = await DB._ensureInitialized();
                const dataToSave = {
                    employee_id: debtData.employeeId,
                    client_name: debtData.clientName,
                    date: debtData.date,
                    amount: debtData.amount,
                    description: debtData.description || null,
                    is_paid: debtData.isPaid,
                    paid_date: debtData.paidDate || null
                };
                let resultData, error;
                if (debtData.id) {
                    const response = await client.from('debts').update(dataToSave).eq('id', debtData.id).select().single();
                    resultData = response.data;
                    error = response.error;
                    if (error) handleSupabaseError(error, `debts.update(${debtData.id})`);
                } else {
                    const response = await client.from('debts').insert(dataToSave).select().single();
                    resultData = response.data;
                    error = response.error;
                    if (error) handleSupabaseError(error, "debts.insert");
                }
                return DB.utils.snakeToCamel(resultData);
            } catch (error) {
                handleSupabaseError(error, "debts.save");
            }
        },
        delete: async function(id) {
            try {
                ...
                if (error) {
                    handleSupabaseError(error, `debts.delete(${id})`);
                    return false;
                }
                return true;
            } catch (error) {
